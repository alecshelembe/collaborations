...
// Make sure to change all occurances 
...
const SciencePostsPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/get-science-posts/rules`);
  ...
  const openViewer = (images: string[], index: number) => {
    setViewerImages(images.map(img => ({ uri: `https://cash-crusaders.visitmyjoburg.co.za/${img}` })));
      ...
      <View style={styles.gridContainer}>
        {item.image_url.map((img, i) => (
          <TouchableOpacity key={i} onPress={() => openViewer(item.image_url, i)}>
            <Image
              source={{ uri: `https://cash-crusaders.visitmyjoburg.co.za/${img}` }}

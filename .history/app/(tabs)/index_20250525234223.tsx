...
import { ThemedView } from '@/componPents/ThemedView';
      ...
      <View style={styles.card}>
        <View style={styles.header}>
          {item.profile_image_url ? (
            <Image
              source={{ uri: `${API_BASE_URL}/${item.profile_image_url}` }}
        ...
        {item.images && item.images.length > 0 ? (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageContainer}>
            {item.images.map((img, index) => {
              const imageUri = `${API_BASE_URL}/${img}`;
              return (
                <TouchableOpacity
                  key={index}
                  onPress={() => {
                    setViewerImages(item.images.map(i => ({ uri: `${API_BASE_URL}/${i}` })));

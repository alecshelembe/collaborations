...
const BRAND_BACKGROUND_COLOR = '#1a1a1a'; // Example dark background color for your theme
const API_BASE_URL = 'https://atamela.visitmyjoburg.co.za/api';
...
const imageSpacing = 5; // Gap between images
const space = 14; // Gap between images
...
const SciencePostsPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/get-science-posts`);
  ...
  const openViewer = (images: string[], index: number) => {
    setViewerImages(images.map(img => ({ uri: `${IMAGE_BASE_URL}/${img}` })));

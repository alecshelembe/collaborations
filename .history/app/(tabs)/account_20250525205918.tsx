...
export default function CreateUserWithImage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const response = await axios.post('https://lego-robotics.visitmyjoburg.co.za/api/create-account', formData, {
        headers: {
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        {inputFields.map(({ name, label, ...rest }) => (
          <View key={name} style={styles.inputGroup}>
            <Text style={styles.label}>{`Enter your ${label.toLowerCase()}`}</Text>
            <Controller
              control={control}
              name={name}
              defaultValue=""
